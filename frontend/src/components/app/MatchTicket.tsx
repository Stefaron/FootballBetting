...
export function MatchTicket({
  ...
  const [step, setStep] = useState<"idle" | "approving" | "depositing" | "resolving" | "claiming">("idle");
  ...
  }, [isConfirmed, step, betState.teamId, onClaimSuccess]);
  ...
  const handleDeposit = (teamId: number) => {
    ...
    writeContract({
      ...
      args: [BigInt(teamId), BigInt(10 * 10**18)], // Bet 10 mUSDT
    });
  };
        ...
        <div className="grid grid-cols-2 divide-x divide-white/10 relative">
            ...
            <div className="flex flex-col items-center p-6 text-center group/a">
                ...
                {!betState.hasBet ? (
                  ...
                  <div className="w-full py-3 font-space text-xs text-white/40 uppercase text-center">
                    Bet Placed on {teamB}
                  </div>
                )}
            </div>
